...
namespace ORB_SLAM3 {
    ...
    void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, std::vector<ORB_SLAM3::MapPoint *> &vpMP,
        ...
        if (hasAlignment && GPSGlobal) {
            ...
            // Find the first KeyFrame in the optimizer (usually ID 0 or the first in the list)
            // and ensure it is NOT fixed so the whole map can shift to the GPS coordinates.
            for (auto pKF: vpKFs) {
                g2o::VertexSE3Expmap *vKF = static_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(pKF->mnId));
                if (vKF) vKF->setFixed(false);
            }
        }
            ...
            if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
                ...
                if (pKFi->bImu && pKFi->mPrevKF->bImu) {
                    ...
                    if (!bInit) {
                        ...
                        ear->setVertex(0, VA1);
                        ear->setVertex(1, VA2);
                        Eigen::Matrix3d InfoA = pKFi->mpImuPreintegrated->C.block<3, 3>(12, 12).cast<double>().
                                inverse();
            ...
            for (map<KeyFrame *, tuple<int, int> >::const_iterator mit = observations.begin(), mend = observations.end()
                ...
                if (!pKFi->isBad()) {
                    ...
                    if (leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)] < 0) // Monocular observation
                    {
                        ...
                        g2o::OptimizableGraph::Vertex *VP = dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.
                            vertex(pKFi->mnId));
                        if (bAllFixed)
                            if (!VP->fixed())
                                bAllFixed = false;
                ...
                if (pMP) {
                    ...
                    //SLAM with respect a rigid body
                        ...
                        if (i < pFrame->Nleft) {
                            ...
                            ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody *e = new
    ...
    void Optimizer::OptimizeEssentialGraph(KeyFrame *pCurKF, vector<KeyFrame *> &vpFixedKFs,
        ...
        for (KeyFrame *pKFi: vpKFs) {
            ...
            for (vector<KeyFrame *>::const_iterator vit = vpConnectedKFs.begin(); vit != vpConnectedKFs.end(); vit++) {
                KeyFrame *pKFn = *vit;
                if (pKFn && pKFn != pParentKFi && !pKFi->hasChild(pKFn) && !sLoopEdges.count(pKFn) && spKFs.find(pKFn)
                    != spKFs.end()) {
    ...
    void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF,
        ...
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            ...
            if ((e->chi2() > chi2Mono2 && !bClose) || (e->chi2() > 1.5f * chi2Mono2 && bClose) || !e->
                isDepthPositive()) {
                KeyFrame *pKFi = vpEdgeKFMono[i];
                vToErase.push_back(make_pair(pKFi, pMP));
            }
    ...
    Eigen::MatrixXd Optimizer::Marginalize(const Eigen::MatrixXd &H, const int &start, const int &end) {
        ...
        Hn.block(0, 0, a + c, a + c) = Hn.block(0, 0, a + c, a + c) - Hn.block(0, a + c, a + c, b) * invHb * Hn.block(
                                           a + c, 0, b, a + c);
    ...
    void Optimizer::AddGpsEdge(KeyFrame *pKF, g2o::SparseOptimizer *optimizer) {
        if (!pKF->HasGPS() || !hasAlignment) {
            return;
        }
        g2o::VertexSE3Expmap *vKF = static_cast<g2o::VertexSE3Expmap *>(optimizer->vertex(pKF->mnId));
        ...
        g2o::RobustKernelHuber *rk = new g2o::RobustKernelHuber;
        e->setRobustKernel(rk);
        rk->setDelta(GPSHuberDelta / T_init.scale()); // Threshold in meters

        optimizer->addEdge(e);
    }

    void Optimizer::RecalculateGpsTransformation(const vector<KeyFrame *> &vpKF) {
        std::vector<Eigen::Vector3f> points_slam;
        std::vector<Eigen::Vector3d> points_gps;
        for (size_t i = 0; i < vpKF.size(); i++) {
            KeyFrame *pKF = vpKF[i];
            if (pKF->HasGPS()) {
                points_slam.push_back(pKF->GetCameraCenter());
                points_gps.push_back(pKF->mGPSPositionENU);
            }
        }
        bool succ = false;
        g2o::Sim3 res = GPSUtils::CalculateSim3Alignment(points_slam, points_gps, &succ);
        if (succ) {
            T_init = res;
            T_sim3_gps_local = T_init.inverse();
            hasAlignment = true;
            cout << "Recalculated GPS Transformation: scale: " << T_init.scale() << endl;
        }
    }
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pMainKF, vector<KeyFrame *> vpAdjustKF,
        ...
        for (unsigned int i = 0; i < vpMPs.size(); ++i) {
            ...
            for (map<KeyFrame *, tuple<int, int> >::const_iterator mit = observations.begin(); mit != observations.end()
                 ; mit++) {
                KeyFrame *pKF = mit->first;
                if (pKF->isBad() || pKF->mnId > maxKFid || pKF->mnBALocalForMerge != pMainKF->mnId || !pKF->GetMapPoint(
                        get<0>(mit->second)))
                    continue;
        ...
        Verbose::PrintMess(
            "[BA]: Second optimization, there are " + to_string(badMonoMP) + " monocular and " + to_string(badStereoMP)
        ...
        for (unsigned int i = 0; i < vpMPs.size(); ++i) {
            ...
            for (map<KeyFrame *, tuple<int, int> >::const_iterator mit = observations.begin(); mit != observations.end()
                 ; mit++) {
                KeyFrame *pKF = mit->first;
                if (pKF->isBad() || pKF->mnId > maxKFid || pKF->mnBALocalForKF != pMainKF->mnId || !pKF->GetMapPoint(
                        get<0>(mit->second)))
                    continue;
    ...
    void Optimizer::MergeInertialBA(KeyFrame *pCurrKF, KeyFrame *pMergeKF, bool *pbStopFlag, Map *pMap,
        ...
        // Fixed Keyframes. Keyframes that see Local MapPoints but that are not Local Keyframes
        int i = 0;
        for (vector<pair<MapPoint *, int> >::iterator lit = pairs.begin(), lend = pairs.end(); lit != lend;
             lit++, i++) {
